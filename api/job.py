...
class JobAPI:       
    ...
    class _applicationSortingAlgorithm(Resource):
        def get(self):
            applications = Application.query.filter_by(jobid=request.args.get('id')).all()
 

            def bubble_sort_by_experience(applications):
                n = len(applications)
                # outer loop to make multiple passes over the list
                for i in range(n): 
                    # inner loop to compare and swap elements
                    for j in range(0, n-i-1):  
                        # if statement compares the years of experience of current application to years of experience of next application
                        # if the current application has less experience than the next one, they are swapped
                        # this ensures that applications with more experience move towards the beginning of the list.
